...
این دستور:
- پوشه `db/` را در ریشه پروژه ایجاد می‌کند
- فایل `.env` را با مسیر absolute دیتابیس به‌روزرسانی می‌کند (cross-platform: Windows، macOS، Linux)
- Prisma Client را generate می‌کند
- جدول‌ها را در SQLite ایجاد می‌کند

> **مهم برای کاربران ویندوز:** این اسکریپت به‌طور خودکار مسیر absolute را با `/` (نه `\`) تولید می‌کند که Prisma به آن نیاز دارد. نیازی به تنظیم دستی `.env` نیست.

> **اگر خطای `Error code 14: Unable to open the database file` دریافت کردید:**
> این یعنی فایل `.env` شما مسیر اشتباه دارد. کافیست `npm run db:setup` را دوباره اجرا کنید تا مسیر به‌طور خودکار اصلاح شود.

### 4. ایجاد فایل `.env` (در صورت نیاز)

اگر فایل `.env` وجود ندارد، آن را از `.env.example` کپی کنید:

```bash
cp .env.example .env
```

سپس `npm run db:setup` را اجرا کنید تا مسیر دیتابیس به‌طور خودکار تنظیم شود.
